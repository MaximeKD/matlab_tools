...
function diff_channel = compareFiles(filepath1, filepath2, channelfile1, channelfile2, fig, freqResp, figureName)
...
if nargin < 7 || isempty(figureName)
    customTitle = 0;
else
    customTitle = 1;
end
...
max_diff_channel = max(abs(diff_channel));
max_legend = ['Max Diff : ', num2str(max_diff_channel)];
